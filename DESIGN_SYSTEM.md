...
Resting tiles use a light neutral surface and low-opacity dot texture. Hover activates a brand-led vertical gradient, a subtle white edge, a soft shadow, and a restrained lift while retaining the dotted texture. Crossfade from the resting logo to the approved active-state asset over roughly `400–500ms`; do not recolor a full rectangular image with CSS filters.

Each tile reveals an experience tooltip below the card on hover or keyboard focus. Tooltips use a `417px` white surface, `24px` padding, a `16px` radius, a subtle border, and a soft single shadow. Use Public Sans for both the `14px/20px` summary and muted `16px/22px` company label. Keep the summary concise enough to scan without interacting with the panel.
